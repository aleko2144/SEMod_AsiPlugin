...
typedef void(*xz)(int a1, int a2, int a3, int a4, int a5, int a6, int a7);
DllClass* instance;
float *x;
float *m;
float kilometrage;
...
void BlockSpace_Move(int *address, Vector3D pos, int type)
{
	float *Matrix = (float *)(address + 16);
...
Vector3D BlockSpace_GetPosition(int *address)
{
	float *Matrix = (float *)(address + 16);
	Vector3D pos;
	pos.x = Matrix[9];
...
Vector3D BlockSpace_GetAngles(int *address)
{
	float *Matrix = (float *)(address + 16);
...
float *BlockSpace_GetMatrix(int *address)
{
	float *Matrix = (float *)(address + 16);
	return Matrix;
}

void BlockSpace_Rotate(int *address, Vector3D rot, int type)
{
	float *Matrix = (float *)(address + 16);
	...
	if (Viewer != 0) //*(DWORD *)0x6F6938 //(int)0x6D2098 != 0
	{
		...
		if (airMode == 1){
			...
			*(float *)(Car_V + 0xD0 + 8) = 0;
			...
			if (k50BS != 0){
				car_rot = BlockSpace_GetAngles((int *)BlockSpace);
...
void A_Interiors()
{
	if (Viewer != 0){
		...
		if (ECOSpaceI != 0)
		{
			...
			BlockSpace_Rotate(ECOSpaceI, eco, 0);
		}
		...
		if (AKBSpaceI != 0)
		{
			...
			BlockSpace_Rotate(AKBSpaceI, AKB, 0);
		}
		...
		if (FuelSpaceI != 0)
		{
			...
			BlockSpace_Rotate(FuelSpaceI, Fuel, 0);
		}
	}
}
...
void Panel()
{
	...
	{	
		...
		if (SpeedSpaceAddress != 0){
			...
			BlockSpace_Rotate(SpeedSpaceAddress, speed_v, 0);
		}
		if (TachoSpaceAddress != 0){
			Vector3D tacho_v; tacho_v.x = 0; tacho_v.y = _tacho; tacho_v.z = 0;
			BlockSpace_Rotate(TachoSpaceAddress, tacho_v, 0);
		}
		if (FuelArrowSpaceAddress != 0){
			Vector3D fuel_v; fuel_v.x = 0; fuel_v.y = _fuelArrow; fuel_v.z = 0;
			BlockSpace_Rotate(FuelArrowSpaceAddress, fuel_v, 0);
		}
		//
