...
inline int *GetSceneObject(int *parent, const char *name)
{
...
inline int *GetSceneObjectLogged(int *parent, char *name, char *a3) //если нет объекта, то игра завершит свою работу
{
...
inline void SetCaseSwitch_(int *offset, int state)
{
	typedef int(*SetCaseSwitch)(int *offset, int state);
	int ret = SetCaseSwitch(0x4E48AC)(offset, state);
}

inline void SetCaseSwitch(int* offset, int state){
